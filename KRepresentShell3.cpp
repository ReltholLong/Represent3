...
bool Test3D()
{
	// Thay vì dùng DirectDraw, dùng Direct3D
	IDirect3D9* pD3D = Direct3DCreate9(D3D_SDK_VERSION);
	if (!pD3D) return false;

	// Check adapter info thay vì DirectDraw caps
	D3DADAPTER_IDENTIFIER9 adapter;
	pD3D->GetAdapterIdentifier(D3DADAPTER_DEFAULT, 0, &adapter);
	pD3D->Release();

	// Check system memory instead
	MEMORYSTATUS stat;
...
{
	uint32 nPixelCount = 0;							// ×Ü×Ö½Ú¼ÆÊý
	uint32 nTotlePixel = nWidth * nHeight;
    uint32 nNextWidth = nWidth;
	uint32 pixelNum;
	BYTE *pTexLine = (BYTE*)pDest;					// ÌùÍ¼Ã¿Ò»ÐÐÊý¾Ý
...
// fd2,fd2Ä¿±êÇøÓò£¬fs1,fs2Ô­ÇøÓò£¬µãÔÚÔ­ÇøÓòÖÐµÄÎ»ÖÃ
...
// °´fsÏà¶ÔÓÚfs1ºÍfs2µÄÎ»ÖÃ¶Ôv1ºÍv2½øÐÐ²îÖµ
...
void KRepresentShell3::DrawPlayer3D(int nPrimitiveCount, KRepresentUnit* pPrimitives, int bSinglePlaneCoord)
{
	...
	// ¼ÆËã·ÇÍ¸ÊÓÄ£Ê½ºÍÍ¸ÊÓÄ£Ê½µÄÍâ°ü¾ØÐÎ
	...
	int nY, unsigned int Color, int nLineWidth, int nZ, unsigned int BorderColor)
{
	...
	int i = 0;  // Khởi tạo luôn
	for (i = 0; i < RS2_MAX_FONT_ITEM_NUM; i++)  // Bỏ "int" ở đây
	{
	...
	const char* psText, int nCount, int nLineWidth)
{
	...
	if (fontIndex >= 0 && m_FontTable[fontIndex].pFontObj)
	{
		KTextProcess	tp(psText, nCount, nLineWidth * 2 / nFontId);
		if (pParam->nZ != TEXT_IN_SINGLE_PLANE_COORD)
		{
		...
		m_FontTable[fontIndex].pFontObj->SetBorderColor(pParam->BorderColor);
		m_FontTable[fontIndex].pFontObj->SetOutputSize(nFontId, nFontId + 1);
		return tp.TransXYPosToCharOffset(nX, nY, m_FontTable[fontIndex].pFontObj, nFontId, pParam);
	}
	return -1;
}
